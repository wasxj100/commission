...
BY = conf.get("by_sorting","by_what")
...
def get_db_connection():
    ...
    if 'conn' not in globals() or conn is None:
        conn = sqlite3.connect("commission.db", check_same_thread=False)
    return conn
...
def save_or_update(date_str, data_dict):
    ...
    if record_id:
        updates = [f"{k}=?" for k in data_dict.keys()]
...
class DataQueryPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.y.setRange(2000, 2100)
        self.y.setValue(QDate.currentDate().year())
        self.y.setFixedHeight(36)

        self.m = QSpinBox()
        self.m.setRange(1, 12)
        self.m.setValue(QDate.currentDate().month())
        self.m.setFixedHeight(36)

        self.btn_query = QPushButton("查询")
        self.btn_query.clicked.connect(lambda :self.query(BY))
    ...
    def query(self,by_sorting):
        ...
        if df.empty:
            ...
            return
        _df = df.sort_values(by=f"{by_sorting}", ascending=True)
    ...
    def delete_row(self):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM records WHERE id=?", (id_item.text(),))
        conn.commit()
